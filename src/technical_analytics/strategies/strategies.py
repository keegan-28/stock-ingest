...
def macd(df: pl.DataFrame, fast: int = 12, slow: int = 26, signal: int = 9) -> pl.DataFrame:
    df = ema(df, fast, "close", "ema_fast")
    df = ema(df, slow, "close", "ema_slow")

    df = df.with_columns((pl.col("ema_fast") - pl.col("ema_slow")).alias("macd"))

    # Calculate the signal line (EMA of the MACD line)
    df = ema(df, signal, "macd", "macd_signal")
...
def bollinger_bands(
    ...
    df = rolling_mean(df, window_size, "close", f"{output_prefix}_ma")
    df = rolling_std(df, window_size, "close", f"{output_prefix}_std")
    df = df.with_columns(
        [
